  ...
  (props, ref) => {
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
      ...
      if (e.key === "Tab" && !bangMode && value.trim()) {
        ...
        // 查找匹配的关键词
        const matchedBangName = Object.keys(bangConfig).find(
          (bangName) => bangConfig[bangName as BangName] === value.trim(),
    ...
    const searchEngineName = SEARCH_ENGINE_OPTIONS.find(
      (item) => item.value === userOptions?.searchEngine,
    )?.label;
    ...
    return (
      <div className="flex items-center gap-3 w-full px-2">
        ...
        <input
          ...
          placeholder={
            bangMode
              ? t("bangSearchPlaceholder", { name: bangMode.name })
              : t("searchPlaceholder")
          }
        />
