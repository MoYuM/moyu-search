...
// 检查是否为扩展上下文失效错误
// TODO：这里去掉吧
export const isExtensionContextError = (
  error: any,
): error is ExtensionContextError => {
  return (
...
export const safeSendToBackground = async <T = unknown>(
    ...
    if (isExtensionContextError(error)) {
      ...
      if (config.retry && !isMaxRetriesExceeded()) {
        ...
        await new Promise((resolve) =>
          setTimeout(resolve, config.retryDelay * getRetryCount()),
        );

        // 请求 background script 重新注入 content script
        try {
          await chrome.runtime.sendMessage({
            type: ExtensionMessageType.REINJECT_CONTENT_SCRIPT,
          });
          // 重置状态并重试
          resetExtensionContext();
          return await safeSendToBackground<T>(message, {
            ...config,
            retry: false,
          });
        } catch (reinjectError) {
          console.error(
            "Failed to request content script reinjection:",
            reinjectError,
          );
        }
      }
      ...
      throw new Error(
        "Extension context invalidated, content script will be reinjected",
      );
    }
    throw error;
  }
...
export const safeSendToBackgroundSimple = async <T = unknown>(
  message: ExtensionMessage,
): Promise<T> => {
